...
The system SHALL validate scalar hospital fields for type, length, and format before create.

#### Scenario: Email is invalid
- **WHEN** a create request contains an invalid email address
- **THEN** validation fails with HTTP 422

#### Scenario: Numeric count is negative
- **WHEN** a create request contains a negative bed count or employee count
- **THEN** validation fails with HTTP 422
...
- **WHEN** a create request contains an unknown JSON field
